...
# Additional feature flags and behaviors
...
REGEN_WEEKS = set([w.strip() for w in os.getenv('REGEN_WEEKS', '').split(',') if w.strip()])
RESET_HASH_HISTORY = _env_bool('RESET_HASH_HISTORY', False)
RESET_WR_LIST = set([w.strip() for w in os.getenv('RESET_WR_LIST', '').split(',') if w.strip()])

# Hash history for skip logic
HASH_HISTORY_PATH = os.path.join(OUTPUT_FOLDER, 'hash_history.json')

def load_hash_history(path: str) -> dict:
    try:
...
def save_hash_history(path: str, data: dict):
    try:
    ...
    for row in sheet.rows:
        ...
        for att in attachments:
            ...
            if ident not in valid_wr_weeks:
                try:
                    # Smartsheet SDK delete by attachment id
                    client.Attachments.delete_attachment(att.id)
                    removed += 1
...
def delete_old_excel_attachments(client, target_sheet_id, target_row, wr_num, week_raw, current_data_hash, force_generation: bool = False):
    ...
    try:
        ...
        return 0, False
    # Only consider attachments for this specific week to avoid deleting other weeks
    ...
    for a in attachments:
        ...
        if not name.lower().endswith('.xlsx'):
            continue
        if name == f"{prefix}.xlsx" or name.startswith(prefix + "_"):
            excel_attachments.append(a)
    ...
    if not force_generation:
        for att in excel_attachments:
            ...
            if existing_hash and existing_hash == current_data_hash:
                logging.info(f"⏩ Data unchanged for WR# {wr_num} Week {week_raw} (hash {current_data_hash}); skipping regeneration & upload")
                skipped_due_to_same_data = True
                return 0, True
    else:
        logging.info(f"⚐ FORCE REGEN for WR {wr_num} Week {week_raw} — ignoring existing hash match")

    logging.info(f"🗑️ Deleting {len(excel_attachments)} old Excel attachment(s) for WR# {wr_num} Week {week_raw}")
    for att in excel_attachments:
        try:
            # Smartsheet SDK: delete by attachment id
            client.Attachments.delete_attachment(att.id)
...
def _has_existing_week_attachment(client, target_sheet_id: int, target_row, wr_num: str, week_raw: str) -> bool:
    """Check if a row already has an attachment for given WR/week identity."""
    try:
        attachments = client.Attachments.list_row_attachments(target_sheet_id, target_row.id).data
    except Exception:
        return False
...
def purge_existing_hashed_outputs(client, target_sheet_id: int, wr_list: set | None, test_mode: bool):
    """Delete all WR_*.xlsx attachments, optionally limited to specified WRs."""
    try:
        sheet = client.Sheets.get_sheet(target_sheet_id)
    except Exception as e:
        logging.warning(f"⚠️ Unable to load sheet for purge: {e}")
        return
    for row in sheet.rows:
        try:
            attachments = client.Attachments.list_row_attachments(target_sheet_id, row.id).data
        ...
        for att in attachments:
            name = getattr(att, 'name', '') or ''
            if not (name.startswith('WR_') and name.lower().endswith('.xlsx')):
                continue
            if wr_list:
                parts = name.split('_')
                if len(parts) >= 2 and parts[0] == 'WR':
                    wr = parts[1]
                    if wr not in wr_list:
                        continue
            try:
                if not test_mode:
                    client.Attachments.delete_attachment(att.id)
                logging.info(f"🧨 Purged attachment: {name}")
            except Exception as e:
                logging.warning(f"⚠️ Failed to purge {name}: {e}")
...
def main():
    ...
    # Track how many history entries we updated this run
    history_updates = 0
