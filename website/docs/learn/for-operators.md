...
| The file name says `_NO_MATCH` or `Unknown_Foreman` | The robot could not work out a foreman for the row | Fill in the foreman so future rows are right, **and escalate straight away**: a report that has already been built as `Unknown_Foreman` keeps that name on every later run — the attribution is frozen (first write wins) and only engineering can correct it, so waiting for the next run changes nothing. The name itself does **not** stop the upload: if the Work Request has a row on a target sheet the file is attached under that name (if the WR is on no target sheet the file is built but withheld) |
