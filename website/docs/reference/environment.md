...
| `TIME_BUDGET_MINUTES` | `0` (code) / `165` (workflow) | Graceful stop budget in minutes. `0` disables the early-exit. The weekly workflow sets `165` (2h45m) with a matching runner `timeout-minutes: 180` (15min cushion for cache/artifact save steps); local runs default to disabled. Raised 2026-05-26 from `95`/`110` for headroom while the attribution pre-pass scope (below) brings normal runtime back down. |
| `ATTRIBUTION_RESOLUTION_WEEKS` | `8` | Recent-week scope for the per-row frozen-attribution pre-pass (B/C/D claim attribution + the subcontractor-helper path). Each completed row triggers one `lookup_attribution` Supabase RPC; run unbounded this resolved **every completed row across all historical weeks** (~137k RPCs/run observed), dominated by old weeks that change-detection skips anyway — which blew the time budget. This gate resolves attribution only for rows whose `week_ending` is within the last N weeks of today, so pre-pass cost tracks **active** work, not total history. **Safe:** an out-of-scope row resolves to use-current at emission, but its group is either (1) unchanged + attachment exists → skipped, claimer unused; or (2) the rare edit to a `>N`-week-old row → regenerated with the current foreman (the same legacy/no_history fallback the feature already documents). The **freeze (write) side is untouched** — every completed row is still frozen during generation — so durable attribution data stays complete. Set `0` to disable scoping (resolve every week; pre-hotfix behavior). Default `8` (~2 months) is a generous active-billing horizon; lower if runs still approach the budget, raise if old-week edits must keep frozen attribution. Resolved value logged at startup as `📋 ATTRIBUTION_RESOLUTION_WEEKS=<n>`. (Added 2026-05-26.) |
