...
- **No new commands, env vars or schedule changes.** `HELPER2_ENABLED`, `EXCLUDE_WRS`, `WR_FILTER`, `REGEN_WEEKS` and `RESET_WR_LIST` keep their meanings; the difference is that they now cover Helper #2 files too.
- **Expect one regeneration wave** of `_AEPBillable_Helper2_*` / `_ReducedSub_Helper2_*` workbooks on the first scheduled run after merge: their content hash changes because the priced totals change. Helper #1, primary and VAC-crew files are byte-identical to before.
