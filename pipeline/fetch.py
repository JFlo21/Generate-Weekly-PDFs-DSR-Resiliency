    ...
    def _fetch_and_process_sheet(source):
        ...
        try:
            ...
            try:
                ...
                # Join into a comma-separated string so the Smartsheet API
                # receives a single ?columnIds=1,2,3 parameter.  Passing a
                # Python list causes requests to emit repeated params
                # (?columnIds=1&columnIds=2) which the API silently ignores,
                # returning all columns and triggering error 4000 on large sheets.
                ...
                if _is_auth_api_error(e):
                    ...
                    logging.debug(f"Auth error (401/403) processing sheet {source['id']} — will consolidate: {e}")
                else:
            ...
            if _is_auth_api_error(e):
                auth_error_sheet_ids.append(source.get('id'))
                logging.debug(f"Auth error (401/403) accessing Sheet ID {source.get('id', 'N/A')} — will consolidate: {e}")
            else:
