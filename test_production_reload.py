#!/usr/bin/env python3
"""
Test the actual production group_source_rows function with module reload.
This forces a fresh import of the module after each environment change.
"""
...
def excel_serial_to_date(value):
    ...
    return datetime.datetime(2023, 12, 3)

def run_production_test(mode):
    """Test the actual production grouping logic with forced module reload."""
    print(f"\n{'='*60}")
    ...
    with patch('generate_weekly_pdfs.excel_serial_to_date', excel_serial_to_date):
        ...
        group_source_rows = generate_weekly_pdfs.group_source_rows
        ...
        if mode == 'both':
            ...
            if len(primary_groups) != 1:
                ...
                print(f"  ❌ Expected 3 rows in main (should include helper row), got {len(groups[primary_groups[0]])}")
                print(f"     Foremen in main: {[r.get('Foreman') for r in groups[primary_groups[0]]]}")
                test_passed = False
                
            if len(helper_groups) != 0:
                print(f"  ❌ Expected 0 helper groups in primary mode, got {len(helper_groups)}")
                test_passed = False
...
def main():
    """Run production tests with module reload."""
    print("🧪 Testing PRODUCTION Helper Row Exclusion Logic (with module reload)")
    print("="*60)
    print("This tests the actual group_source_rows function with forced module reload")
    print()
