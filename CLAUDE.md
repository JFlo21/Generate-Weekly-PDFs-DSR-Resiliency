...
- [2026-05-27 16:45] **Legacy-hash cross-claimer attachment cleanup**
  (debug session ``legacy-attachment-cleanup``). After the 2026-05-27
  Supabase RPC contract fix restored correct frozen-claimer resolution,
  the workflow regenerates the CORRECT per-week claimer file (e.g.
  ``_ReducedSub_User_Mark_Diaz``) but the OLD wrong-claimer LEGACY
  HASH-NAMED file from the broken period (e.g.
  ``_ReducedSub_User_Wade_Watson_<ts>_<hash>``) was NEVER deleted, so
  duplicates accumulated on ``TARGET_SHEET_ID`` and
  ``SUBCONTRACTOR_PPP_SHEET_ID``. **Root cause:**
  ``cleanup_untracked_sheet_attachments`` dedups ONLY within a single
  identity tuple ``(wr, week, variant, claimer)`` — it keeps the newest
  copy per identity and never cross-deletes between DIFFERENT claimers.
  This is the deliberate Foundation A **"claimer-file coexistence /
  no-cross-delete" invariant** ([2026-05-20 13:45] rule 2): a legitimate
  mid-week foreman switch must yield TWO coexisting files, neither
  clobbering the other. When the attribution fix corrected a historical
  claimer (Wade Watson -> Mark Diaz), the run ADDED the correct
  ``_User_Mark_Diaz`` (clean-named, Sub-project E scheme) file but the
  old ``_User_Wade_Watson_<ts>_<hash>`` file is a DIFFERENT identity with
  one member -> kept. No existing path cross-deleted it: the per-identity
  dedup skipped it (different identity); ``run_claimer_remediation`` only
  sweeps ``_NO_MATCH`` / ``_Unknown_Foreman`` (not a real-but-wrong name);
  a forced full regen still wouldn't cross-delete a wrong-claimer file.
  **Fix (additive, kill-switch-gated):** new default-OFF env var
  ``LEGACY_HASH_CLAIMER_CLEANUP_ENABLED`` (truthy ``1``/``true``/``yes``/
  ``on``) + a new ``legacy_hash_cleanup: bool = False`` param on
  ``cleanup_untracked_sheet_attachments``. When ON, AFTER the existing
  within-identity dedup, for each ``(wr, week, variant)`` that has a
  CLEAN-named current attachment on the row (the supersession signal),
  the surviving LEGACY HASH-NAMED attachment for that same
  ``(wr, week, variant)`` — REGARDLESS of claimer — is deleted. The
  hash/timestamp token is the reliable legacy discriminator: a new helper
  ``_is_legacy_hash_named(filename)`` returns True iff the token
  IMMEDIATELY AFTER ``WeekEnding_<week>`` is a 6-digit numeric ``HHMMSS``
  timestamp (the SAME structural signal ``build_group_identity`` uses to
  tell legacy token-bearing names from clean ones; clean names never
  carry it). **The no-cross-delete invariant is intact:** two CLEAN-named
  claimers for the same ``(wr, week, variant)`` are BOTH clean, so neither
  is ``_is_legacy_hash_named`` -> neither is deletion-eligible; a legacy
  file with NO clean sibling is kept (only within-identity dedup acts on
  it). Wired at BOTH the TARGET and PPP call sites; PPP's
  ``variant_whitelist={'reduced_sub','reduced_sub_helper'}`` prunes
  off-contract variants BEFORE the cross-claimer gate, so the cleanup only
  ever acts on whitelisted reduced_sub* legacy duplicates there. New
  counter ``removed_legacy_hash`` surfaced in the
  ``🧹 Variant pruning done:`` summary log; PII marker
  ``"Removed legacy hash-named cross-claimer attachment"`` registered in
  ``_PII_LOG_MARKERS``. **Operator decisions:** (1) the ~147 unattributable
  rows (source foreman ``#NO MATCH`` / blank, regenerating as
  ``_User__NO_MATCH`` / ``_User_Unknown_Foreman``) are LEFT to the existing
  ``run_claimer_remediation`` sweep — this migration is scoped to
  wrong-claimer HASH-named files ONLY; no skip-at-source or new garbage
  logic was added (clean separation of concerns). (2) The kill switch is
  INDEPENDENT of ``SUPABASE_HASH_STORE_AUTHORITATIVE`` (default OFF, its
  own switch) so it runs as a PREP step BEFORE E is flipped to ``'1'`` —
  avoiding the chicken-and-egg (E can't activate while wrong-claimer
  duplicates linger, but an E-gated cleanup couldn't run until E is
  active). **Activation:** via the ``advanced_options`` workflow_dispatch
  field (``legacy_hash_cleanup:1``), exported to ``$GITHUB_ENV`` by the
  parser; NO literal step-``env:`` pin (per the WR-02 rule a literal pin
  would mask ``$GITHUB_ENV``); the Python default ``'0'`` keeps cron runs
  OFF. **New rules:** (1) When an upstream correction (attribution fix,
  source-data fix) changes the IDENTITY a group resolves to, the prior
  identity's attachment becomes an orphan that the per-identity dedup will
  NEVER remove (it is a different identity). A migration that removes such
  cross-identity orphans MUST use a structural "this is the OLD scheme"
  signal (here: hash-named vs clean-named) as the deletion gate, and MUST
  require a current/correct-scheme sibling to exist for the same
  ``(wr, week, variant)`` before deleting — never cross-delete two files
  that are BOTH in the current scheme (that would break the no-cross-delete
  invariant for legitimate multi-claimer weeks). (2) A reusable
  ``_is_legacy_hash_named`` discriminator MUST mirror
  ``build_group_identity``'s leftmost-strong-candidate selection so a
  pathological identifier containing ``WeekEnding_<6digits>`` cannot
  false-positive — the structural delimiter always precedes any
  identifier-internal candidate in a generated filename.
  **Sequencing:** this cleanup is the prep step in the E re-activation
  runbook (deploy RPC done -> remediate ``_NO_MATCH`` garbage + run this
  legacy-hash cleanup -> validate -> flip ``E='1'`` with
  ``RESET_HASH_HISTORY=1`` for a one-time clean+correct regen). Regression
  tests (end-to-end, drive the REAL ``cleanup_untracked_sheet_attachments``
  per project rule [2026-05-20 00:26] #4 — static mirror tests do not
  count): new file ``tests/test_legacy_hash_claimer_cleanup.py``
  (``TestLegacyHashNamedDetector``, ``TestLegacyHashCrossClaimerCleanup``,
  ``TestNoCrossDeleteInvariantPreserved``, ``TestKillSwitchOff``,
  ``TestWithinIdentityDedupStillWorks``, ``TestPppRespectsWhitelist``,
  ``TestEnvFlagWired`` — 14 methods). The signature-guard
  ``TestPppCleanupUntrackedAttachments::test_cleanup_function_signature_unchanged``
  in ``tests/test_security_audit_followup.py`` was reconciled in-place
  (appends ``legacy_hash_cleanup`` as a bool toggle defaulting ``False``,
  not a None-defaulted scope kwarg) per the [2026-05-25 16:30]
  test-contract reconciliation rule. ``pytest tests/`` -> **1000 passed /
  29 skipped / 69 subtests** (was 986 / 29 / 69; +14 net, zero
  regressions). ``python -m py_compile generate_weekly_pdfs.py`` clean.
