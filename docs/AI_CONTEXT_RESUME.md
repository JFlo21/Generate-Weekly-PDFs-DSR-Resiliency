...
> **2026-09-10 pointer (live status lives in `.claude/project-state.md`):** Phase 14 "Foreman Helper #2" is COMPLETE — 14/14 plans merged and observed (last: plan 14-14, PR #396 → `736141a`; docs #397 → `92c9ed6`), `HELPER2_ENABLED=1` on the scheduled workflow (O-14-D: permanent feature, flag-off = emergency kill switch), O-14-E RESOLVED (registry marker on 121/121 rows, 112/121 registry skips on the next run), code review done (PR #399 → `94f2636`) and its Critical/Warning findings FIXED — PR #402 merged 2026-09-10 → `6e6e1e8` (Helper #2 shadow pricing via the rate matrix, `EXCLUDE_WRS` / `WR_FILTER` Helper #2 shapes, SUB-09 scope, billing-audit probe coordination + degraded breaker label, `SUB_RATES_FP` hash gate). Next, in order: Phase 12 12-06 Task 4 (resume at Task 4 ONLY — see the Phase 12 `.continue-here.md`), then observe the first scheduled run after `6e6e1e8` (the two Helper #2 subcontractor shadows regenerate once via the `SUB_RATES_FP` mix-in, expected empty; no other variant hash moves), then the PR #402 owner follow-ups (`.planning/todos/pending/2026-09-10-pr402-review-followups.md`), then Phase 13 when Juan asks. Details: `.planning/phases/14-foreman-helper-2/14-REVIEW.md`, `14-REVIEW-FIX.md`, `14-DECISIONS.md`.
