...
`group_source_rows` never leaves `__current_foreman` blank, and deliberately NOT applied to the
subcontractor / helper-shadow headers (their raw `Foreman` is the primary crew); hashes and identity
