...
flowchart LR
    subgraph "Triggers"
        SCHED["⏰ Schedule<br/><small>Multiple daily runs + weekly</small>"]
...
| File | Purpose |
|------|---------|
| `generate_weekly_pdfs_complete_fixed.py` | Main production script |
...
```
Primary:  WR_{WR#}_WeekEnding_{MMDDYY}_{timestamp}_{hash}.xlsx
Helper:   WR_{WR#}_WeekEnding_{MMDDYY}_{timestamp}_Helper_{name}_{hash}.xlsx
```
...
TEST_MODE=true python generate_weekly_pdfs_complete_fixed.py

# Force regenerate all
FORCE_GENERATION=true python generate_weekly_pdfs_complete_fixed.py

# Process specific WRs
WR_FILTER="90093002,89954686" python generate_weekly_pdfs_complete_fixed.py
```
...
📖 **Full Documentation:** [README.md](https://github.com/JFlo21/Generate-Weekly-PDFs-DSR-Resiliency/blob/master/README.md)
