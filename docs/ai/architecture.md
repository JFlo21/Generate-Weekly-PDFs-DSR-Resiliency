...
`pipeline_memory.sheet_registry` watermark and carries the `helper2-v1` mapping marker reuses the
stored column mapping, any doubt → full validation, D-11.1-01 + Phase 14), then `pipeline.fetch` pulls ~550 rows in
