...
| WR-01 | `pipeline/orchestrate.py:44` imports `AttachmentParentType` from `smartsheet.models.enums.attachment_parent_type` at module top level, the only import of that SDK-internal enum path anywhere in `pipeline/` (consumed at `orchestrate.py:1287-1297`). The rest of the package wraps SDK model imports in lazy, guarded imports (`pipeline/discovery.py` does so for `Sheet` / `Folder` from `smartsheet.models`) — that is the pattern WR-01 should align to. Risk if the internal SDK path moves/renames in a future SDK version. **Corrected 2026-09-02:** earlier wording (also in ledger `[2026-09-02 17:45]` and `12-02-PLAN.md` line 25) wrongly said discovery.py lazily imports the *same* enum path; it never imports `AttachmentParentType` at all. | low | `pipeline/orchestrate.py` | open — planned fix in Phase 12 Plan 12-02 (align to lazy pattern) | 2026-09-02 |
...
- Last verified: 2026-09-02 — read `pipeline/cleanup.py:89-120` (full `_is_sentinel_identifier`
  function) and `pipeline/orchestrate.py` import block + a repo-wide grep for `AttachmentParentType` (only orchestrate.py imports it; align run 1 verifier finding);
