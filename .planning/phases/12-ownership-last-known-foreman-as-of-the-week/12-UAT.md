...
- gap_id: G-12-3
  ...
  severity: blocker
  test: 3
  root_cause: "scripts/backfill_claim_time_attribution.py::_extract_claimer_from_filename strips only a _<6hex>.xlsx hash tail, but public.artifacts.filename (written by scripts/publish_artifacts_to_supabase.py::_parse_stable) is the stable hash-less name WR_<wr>_WeekEnding_<mmddyy>_User_<name>.xlsx, so the remainder 'Unknown_Foreman.xlsx' survives, is desanitized to 'Unknown Foreman.xlsx' and passes is_sentinel_claimer because the extension defeats the sentinel match. The billing_audit.backfill_attribution RPC does guard the PROPOSED value (STEP 4 validation loop, `IF billing_audit.is_sentinel_value(v_row.value) THEN RAISE EXCEPTION`, shipped in 1f159bc and live since 2026-09-03), but is_sentinel_value normalizes only whitespace, underscores and case — never a trailing file extension — so 'Unknown Foreman.xlsx' scores as a real name in BOTH layers and an --apply would freeze the placeholder as a permanent real name in 4,070 rows / 69 WRs (correction recorded by the gap-closure planner, verified against the SQL file line 320). The 12-01 source-3 fixtures were all hash-suffixed, so the live shape was never exercised. Secondary gaps surfaced by the same review: billing_audit.lookup_attribution_bulk returns NULL for '#'-prefixed values, hiding the 945 '#NO MATCH' rows from targeting; the documented known-good sample WR 19073866 has zero rows in every Supabase store, so ROADMAP SC3 'via backfill_hash_history' is not provable from Supabase (WR 89829163 is a partial substitute pending Juan)."
  artifacts:
    ...
    - path: "billing_audit/own03_backfill_attribution.sql"
      issue: "backfill_attribution RPC already rejects a sentinel proposed value (STEP 4 loop, line 320), but is_sentinel_value is extension-blind, so 'Unknown Foreman.xlsx' passes; an additional extension check is needed without widening is_sentinel_value (it also drives the per-role UPDATE ... WHERE clauses)"
  ...
  missing:
    ...
    - "Add an extension-aware proposed-value guard in _build_apply_payload and an additional extension check in the backfill_attribution RPC validation loop (is_sentinel_value itself stays byte-identical), so a placeholder with a file extension can never be written (owner-applied SQL; production-guardrails Supabase rule)"
