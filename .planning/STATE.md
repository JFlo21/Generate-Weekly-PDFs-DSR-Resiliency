...
- [Phase 14]: O-14-E OPEN (2026-09-09, first enabled run `34356004448`): Helper #2 path verified clean (counters, 0 groups, 0 files, no degrade, freeze_attribution 200s, one-time 218k `row_event` churn) but `sheet_registry.mapping_schema` stays NULL on all 121 rows -- `pipeline/orchestrate.py` never passes `mapping_schema_by_sheet` to `upsert_sheet_registry`, so the D-11.1-01 registry skip is defeated every run (slower but correct). Proposed plan 14-13 awaits Juan.
